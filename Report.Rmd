...
# Creates the inline objects used in the text (never errors if something is missing)
...
# Defaults so the knit won't halt even if models aren't available
flood_intdisp_pct  <- "n/a"; flood_intdisp_sig  <- ""
hurr_intdisp_pct   <- "n/a"; hurr_intdisp_sig   <- ""
drought_asylum_pct <- "n/a"; drought_asylum_sig <- ""
ppml_intdisp_brief <- "n/a"; ppml_asylum_brief  <- "n/a"; intensity_brief <- "n/a"
bp_p_txt <- "p = n/a"; vif_max_txt <- "n/a"

# Fill from your fitted models if they exist
if (exists("m_intdisp_fe")) {
  e1 <- fmt_eff(m_intdisp_fe, "events_flood")
  e2 <- fmt_eff(m_intdisp_fe, "events_hurricane")
  flood_intdisp_pct <- e1$pct; flood_intdisp_sig <- e1$stars
  hurr_intdisp_pct  <- e2$pct; hurr_intdisp_sig  <- e2$stars
}
if (exists("m_asylum_fe")) {
  e3 <- fmt_eff(m_asylum_fe, "lag_drought_events")
  drought_asylum_pct <- e3$pct; drought_asylum_sig <- e3$stars
}

# Brief robustness flags (optional)
if (exists("m_intdisp_ppml")) {
  ct <- try(summary(m_intdisp_ppml)$coeftable, silent = TRUE)
  if (!inherits(ct, "try-error")) {
    sf <- "events_flood"     %in% rownames(ct) && ct["events_flood","Pr(>|z|)"] < 0.1
    sh <- "events_hurricane" %in% rownames(ct) && ct["events_hurricane","Pr(>|z|)"] < 0.1
    ppml_intdisp_brief <- paste0("flood (", if (sf) "sig" else "n.s.", "), hurricane (", if (sh) "sig" else "n.s.", ")")
  }
}
if (exists("m_asylum_ppml")) {
  ct <- try(summary(m_asylum_ppml)$coeftable, silent = TRUE)
  if (!inherits(ct, "try-error")) {
    sd <- "lag_drought_events" %in% rownames(ct) && ct["lag_drought_events","Pr(>|z|)"] < 0.1
    ppml_asylum_brief <- paste0("lagged drought (", if (sd) "sig" else "n.s.", ")")
  }
}
if (exists("m_asylum_intensity")) {
  ct <- try(summary(m_asylum_intensity)$coeftable, silent = TRUE)
  if (!inherits(ct, "try-error")) {
    sf <- "affected_flood_pc"     %in% rownames(ct) && ct["affected_flood_pc","Pr(>|t|)"] < 0.1
    sh <- "affected_hurricane_pc" %in% rownames(ct) && ct["affected_hurricane_pc","Pr(>|t|)"] < 0.1
    intensity_brief <- paste0("affected flood pc (", if (sf) "sig" else "n.s.", "), affected hurricane pc (", if (sh) "sig" else "n.s.", ")")
  }
}

# Optional quick diagnostics (won't error if packages are missing)
if (exists("df")) {
  ols_prev <- try(lm(
    log1p(asylum_apps) ~ lag_drought_events + events_flood + events_hurricane +
      log1p(gdp_pc_const) + agri_va_pct + unemp_rate + remit_gdp_pct + gini_index,
  ...
  if (!inherits(ols_prev, "try-error")) {
    if (requireNamespace("lmtest", quietly = TRUE)) {
      p <- try(lmtest::bptest(ols_prev)$p.value, silent = TRUE)
      if (!inherits(p, "try-error") && !is.na(p)) bp_p_txt <- paste0("p = ", format.pval(p, digits = 3))
    }
    if (requireNamespace("car", quietly = TRUE)) {
      v <- try(car::vif(ols_prev), silent = TRUE)
      if (!inherits(v, "try-error")) vif_max_txt <- sprintf("%.2f", suppressWarnings(max(v, na.rm = TRUE)))
    }
  }
}
